...
if uploaded_file is not None:
 # Can be used wherever a "file-like" object is accepted:
    ...
    if st.button("Predict", use_container_width=True)==0:
        ...
        A = forecast_accuracy(forecast[:12], test.values)
        st.write(A)
        
        Accuracy = 100-( np.mean(np.abs(forecast[:12] - test.values[:(period)])/np.abs(test.values[:(period)])))*100
        MSE = int(np.mean((forecast[:12] - test.values[:(period)])**2)**.5)*100
